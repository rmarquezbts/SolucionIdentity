...
namespace AspNetIdentity.WebApi.Infrastructure
{
    public class RolesFromClaims
    {
        public static IEnumerable<Claim> CreateRolesBasedOnClaims(ClaimsIdentity identity)
        {
            ...
            {
                claims.Add(new Claim(ClaimTypes.Role, "IncidentResolver"));
            }

            //Rol Oficina Autorizada
            if (identity.HasClaim(c => c.Type == "Municipio" && c.Value == "Mexicali"))
            {
                claims.Add(new Claim(ClaimTypes.Role, "Oficina Autorizada"));
            }
